...
Also, Godot uses the positive `x` axis as 0. Rotating to the left produces negative values, and rotating to the right produces positive values:

(radians in red, degrees in blue)

![rotation](/images/rotation.webp)

So, if we want to rotate an object to face up, there are three ways we can do it:

1. Via Editor: setting the **Rotation** property value to `-90`
...
- If we want to convert from degrees to radians: `deg_to_rad(degreevalue)`
...
Translation or movement is obtained by updating the `position` value every frame. `position` is a **Vector2** value relative to its node's parent. `position` can have its value updated by the sum its value with any of these below multiplied by `delta`:

- `transform.x` or `transform.y` multiplied by speed.
- `Vector2.UP` multiplied by speed.
...
func _process(delta):
	...
	if Input.is_action_pressed("ui_up"):
		velocity = Vector2.UP.rotated(rotation) * speed     # Method 1
		velocity = -transform.y * speed                     # Method 2
	velocity = Vector2.UP.rotated(rotation) * Input.get_axis("ui_down", "ui_up") * speed    # Method 3
	velocity = transform.y * Input.get_axis("ui_up", "ui_down") * speed                     # Method 4
	position += velocity * delta
```
